...
impl lumen_runtime::SourceParser for LumencParser {
    ...
    fn resolve_includes(
        ...
        crate::parse::resolve::resolve_includes(
            src,
            self_path,
    ...
    fn collect_fragments(&self, src: &str, self_path: &Path) -> Result<FragmentTable, String> {
        crate::collect_fragments(src, self_path, Some(&crate::parse::resolve::FsLoader))
            .map_err(|e| e.to_string())
    }
    ...
    fn script_refs(&self, src: &str, self_path: &Path) -> Result<ScriptRefs, String> {
        crate::collect_script_refs(src, self_path, Some(&crate::parse::resolve::FsLoader))
            .map_err(|e| e.to_string())
    }

    fn link_fragments(&self, mut table: FragmentTable) -> Result<FragmentTable, String> {
        crate::parse::fragments::link(&mut table).map_err(|e| e.to_string())?;
        Ok(table)
    }
}
