...
pub fn script_fragments(source: &str, uri: &str) -> Result<FragmentTable, String> {
    ...
    for region in lmn::regions(source) {
        ...
        let mut fragment = crate::parse::html::fragment_from_markup(
            &block.markup,
            &block.key,
            &block.args,
            origin,
...
mod tests {
    ...
    fn two_bodies_under_one_key_collide() {
        ...
        let one = crate::parse::html::fragment_from_markup(
            "<label text=\"a\"/>",
            "shared",
            &[],
            origin.clone(),
        ...
        .expect("a body");
        let other =
            crate::parse::html::fragment_from_markup("<label text=\"b\"/>", "shared", &[], origin)
                .expect("a body");
        let mut table = FragmentTable::new();
