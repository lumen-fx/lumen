...
/// `lumen::LUMEN_ABI_{MAJOR,MINOR}` (checked against the loaded library's
/// `lumen_abi_version()` at load time). The launcher does not link `lumen`,
...
pub enum LoaderError {
    ...
    #[error(
        "liblumen ABI mismatch: launcher built against {want_major}.{want_minor}.x, \
         ...
         Rebuild lumen or use a matching liblumen."
    )]
    AbiMismatch {
