...
fn run_css_cascade_lint(dir: &std::path::Path, as_json: bool) -> ExitCode {
    ...
    let sheet = match crate::parse::css::parse_css(&src) {
        Ok(s) => s,
        Err(e) => {
            eprintln!(
    ...
    let findings = crate::parse::css::cascade_lint(&sheet);
    if as_json {
...
/// Static `--signals` lint - defer to [`crate::cli::lint_signals`].
...
fn run_signals_lint(dir: &std::path::Path, as_json: bool, strict: bool) -> ExitCode {
    ...
    crate::cli::lint_signals::cmd_lint_signals(argv.into_iter())
}
