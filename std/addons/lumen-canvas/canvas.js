// The page implementation of the `lumen-canvas` module: the same `canvas`
// functions on the same <canvas> element, drawn with Canvas 2D.
//
...
export function buffer_load_png(path) {
  warn(`buffer_load_png('${path}'): a page has no files to read; load images with the js add-on instead`);
  return 0;
}
