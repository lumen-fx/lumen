//! The `browser` module: what a browser offers a page outside its document: popups and their messages, sharing, downloads, the leave-page prompt, media queries, a file picker, fullscreen.
//!
...
//! `browser::<function> runs only in a browser` in the script that called it, so
...
pub const NAME: &str = "lumen-browser";
...
lumen_module::lumen_module!("lumen-browser", |_config: lumen_module::ModuleConfig| {
    plugin()
});
