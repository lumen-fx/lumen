//! The `js` module: the page's own JavaScript: global functions and values, modules loaded at run time, DOM events.
//!
...
//! `js::<function> runs only in a browser` in the script that called it, so
...
lumen_module::lumen_module!("lumen-js", |_config: lumen_module::ModuleConfig| plugin());
