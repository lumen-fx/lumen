//! The bundled-module shape, end to end: build the `lumen-download` cdylib and
...
//! load modules itself, so everything here runs through the subprocess. The
//! server the subprocess downloads from runs in this process, on a loopback
//! port baked into the script each app carries; nothing reaches the network.
//!
    ...
    F.get_or_init(|| {
        ...
        for line in stdout.lines() {
            ...
            for f in msg["filenames"].as_array().into_iter().flatten() {
                ...
                // The lib target is named `lumen_download`, the spelling the
                // bundled probe derives from `lumen-download`.
                if matches!(name, "lumen_download" | "lumen-download") {
                    download_module = Some(path.clone());
                }
        ...
        let download_module = download_module.expect("bundled download module cdylib built");
        let host = host.expect("fixture host binary built");
        assert_eq!(
            download_module.parent(),
            host.parent(),
            "the download module must build into the host's directory for the bundled probe"
        );

        let scratch = target_dir.join("download-apps");
...
///
/// The host has no way to stop early, so `ticks` is the whole budget a case
/// gets. A case that has to see a transfer through wants a large one: the
/// finished file is flushed to disk before it is renamed into place, and an
/// `fsync` on a loaded machine takes far longer than the transfer it follows.
...
/// function exists in the `download` namespace, and a real transfer from the
/// loopback server lands beside the app, verified against its checksum.
#[test]
fn the_bundled_module_downloads_a_verified_file() {
    let f = fixtures();
...
"#
    ...
    let (stdout, stderr) = run_host(f, &dir, 3000, "started,progress,done,error");

    assert!(
        stdout.contains("HOST loaded name=lumen-download kind=EngineModule build_id=lumen-engine "),
        "{stdout}\n{stderr}"
    );
    assert!(!stdout.contains("HOST failed"), "{stdout}\n{stderr}");
    assert!(stdout.contains("HOST signal started=true"), "{stdout}");
    assert!(
        stdout.contains("HOST signal done=art"),
        "the transfer completed:\n{stdout}\n{stderr}"
    );
    assert!(
        stdout.contains(&format!(
            "HOST signal progress=art:{}/{}",
            BODY.len(),
            BODY.len()
        )),
        "{stdout}"
    );
    assert!(stdout.contains("HOST signal error=<unset>"), "{stdout}");
    assert_eq!(
        std::fs::read(dir.join("payload.bin")).ok().as_deref(),
        Some(BODY),
        "a relative destination named a file beside the app"
    );
    assert!(!stderr.contains("lumen-download:"), "{stderr}");
}

/// A reply that is not 2xx is a failed download: the error handler hears the
/// status, no file appears, and the app carries on.
#[test]
fn a_missing_file_reports_and_leaves_the_app_running() {
    let f = fixtures();
    let server = TestServer::start();
...
"#
    ...
    let (stdout, _stderr) = run_host(f, &dir, 400, "done,error,alive");

    assert!(
        stdout.contains("HOST signal error=") && stdout.contains("HTTP 404"),
        "the failure names the status:\n{stdout}"
    );
    assert!(stdout.contains("HOST signal done=<unset>"), "{stdout}");
    assert!(stdout.contains("HOST signal alive=yes"), "{stdout}");
    assert!(!stdout.contains("HOST tick-panic-caught"), "{stdout}");
    assert!(!dir.join("payload.bin").exists(), "nothing was written");
}

/// The `max_bytes` an app sets in the module's `config` table is what the
/// loaded module enforces.
#[test]
fn the_config_table_bounds_the_body() {
    let f = fixtures();
    let server = TestServer::start();
    let url = server.url("/fixed");
    let dir = write_app(
        f,
        "download-cap",
        "lumen-download = { bundled = true, config = { max_bytes = 8 } }\n",
        &format!(
            r#"
fn on_start() {{ download::to_file("{url}", "payload.bin", "art", ""); }}
fn on_download_progress(tag, received, total) {{}}
...
"#
    ...
    let (stdout, _stderr) = run_host(f, &dir, 400, "done,error");

    assert!(
        stdout.contains("max_bytes"),
        "the refusal names the setting to raise:\n{stdout}"
    );
    assert!(stdout.contains("HOST signal done=<unset>"), "{stdout}");
    assert!(
        !dir.join("payload.bin").exists(),
        "a body past the cap wrote no file"
    );
}
