...
An `.lmna` artifact (produced by `lumenc build`, see the [CLI
reference](cli.md)) carries a container version. This library rejects an
artifact built with a different version rather than risk misreading it, so
an artifact built by an older `lumenc` can stop loading after an update;
rebuild it with the current `lumenc build`. The version most recently
changed to account for a batch of new CSS properties that changed the
