...
to the string the element was written with, then to the key itself. Fallback is
per message, so a partly translated catalogue works: translated messages appear
translated, the rest read in English. The key only stands in for an element's
text, and only when the element has no other translated string: an `<input>`
marked for its placeholder shows an empty field rather than the key.
