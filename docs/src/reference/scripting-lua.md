...
function on_frame(dt)
  signals.angle = signals.angle + dt * 2.0
  request_frame()
end
```
...
`timeout_ms` is a positive number; `credentials` is a string.
...
| `set_locale(tag)` | nil | Switch the app to the BCP-47 locale `tag`. Marked text, placeholders, tooltip bodies, `format` output and the base writing direction follow on the same tick. A locale with no catalogue falls back to the authored text; a tag that is not BCP-47 is logged and ignored. |
...
| Builtin | Returns | Behaviour |
| --- | --- | --- |
| `download.to_file(url, path, tag, checksum)` | boolean | Start downloading `url` to `path`, reporting under `tag`. True once the transfer is running, false when the call was refused. |

Every argument is required. `checksum` is `sha256:` followed by 64 hex digits,
`sha1:` followed by 40, a bare hex string of either length, or an empty string
to check nothing; neither the prefix nor the digits are case sensitive. Any
other spelling fails the call. sha1 is accepted because it is what most package
and artifact registries publish, and the digest here checks a file against a
corrupt transfer rather than standing in for a signature.
...
function on_download_progress(tag, received, total)
  if total > 0 then
    signal("pack_progress", "0"):set(received * 100 // total)
  end
end

function on_download_done(tag, path) signal("pack_path", ""):set(path) end
function on_download_error(tag, message) print("pack failed: " .. message) end
```
...
| Builtin | Returns | Behaviour |
| --- | --- | --- |
| `process.start(cmd, args, tag, opts)` | boolean | Start `cmd` with the argument list `args`, reporting under `tag`. `true` once the program is running. |
| `process.stop(tag)` | boolean | End the program running under `tag`. `false` when nothing runs under it. |
...
looked up on `PATH`. The child reads end of file from its input and has both
its output streams captured.

`opts` is a table of the options below; a key left out takes its default, so
`{}` is every default. A key the options do not have, or a value of the wrong
type, raises before anything starts:
...
| `env` | table of strings | empty | Variables laid over the environment the child inherits. |
| `end_at_exit` | boolean | `false` | End the child when the app exits. |
...
| Builtin | Returns | Behaviour |
| --- | --- | --- |
| `storage.get_item(key)` | string or nil | The stored text, or `nil`. |
| `storage.set_item(key, value)` | boolean | Store text; false when the data directory cannot be written. |
| `storage.remove_item(key)` |  | Remove a key. |
| `storage.keys()` | table | Every key. |
| `storage.clear()` |  | Remove everything. |
| `storage.session_get_item(key)` | string or nil | As `get_item`, in the session set. |
| `storage.session_set_item(key, value)` | boolean | As `set_item`, in the session set. |
| `storage.session_remove_item(key)` |  | As `remove_item`, in the session set. |
| `storage.session_keys()` | table | As `keys`, in the session set. |
...
| Builtin | Returns | Behaviour |
| --- | --- | --- |
| `cookie.get(name)` | string or nil | The cookie's value, or `nil`. A cookie a server set `HttpOnly` is not found. |
| `cookie.set(name, value, options)` | boolean | Set a cookie. Without a `domain` option it stays in the jar for the script alone. |
| `cookie.remove(name, options)` |  | Remove a cookie. `path` and `domain` have to be the ones it was set with. |
| `cookie.keys()` | table | The name of every cookie the script can see. |
...
| Builtin | Returns | Behaviour |
| --- | --- | --- |
| `ws.open(key, url)` | boolean | Open a connection; false when one under `key` is still connecting or open. |
| `ws.send(key, text)` | boolean | Send a text frame; false when the connection is not open. |
| `ws.close(key, code, reason)` | boolean | Close the connection with code 1000, or 3000 to 4999; false when there is none. Another code raises. |
