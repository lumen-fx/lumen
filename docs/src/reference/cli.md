...
| `--lib-dir <dir>` | Directory holding `lumen-web.wasm` and `lumen-web.js`, instead of the published runtime. The web half of a `bundled` module is looked for under `modules/<name>/web/` here first. |
...
a module's [web half](web-addons.md), which runs only in a browser, when
...
emitted as a site, and not one whose web build depends on a module with no
[web half](web-addons.md), whose library a browser cannot load: its
...
app that depends on one emits as a site like any other, and each module's web
half is copied into the site under `addons/<name>.<hash>/`. The web half of a
`bundled` module is looked for under `modules/<name>/web/` in `--lib-dir`, the
directory holding `lumenc`, then `$LUMEN_LIB_DIR`. A web half whose descriptor
is refused fails the build, naming the module.
