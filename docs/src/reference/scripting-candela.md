...
as a call into a module with only a [web half](web-addons.md), which the
...
browser's Canvas 2D. What a script draws in one turn reaches the canvas
together, after the script returns, and the canvas's bitmap follows the
screen's pixel density. `buffer_load_png` and `buffer_save_png` name files,
which a page does not have, so there they answer 0 and false with a console
warning; load an image into a page with the [`js` module](#javascript).
...
| `files::digest(path: string, algo: string)` | `string` | The digest of the file at `path` as lowercase hex, where `algo` is `"md5"`, `"sha1"` or `"sha256"`. A missing file gives an empty string; any other `algo` is refused. |
...
They live in their own `storage` namespace. Text values under text keys, in a
local set that lasts from one run to the next and a session set that lasts as
long as the run. A web build keeps them in the browser's local and session
storage; the desktop keeps the local set in `storage.json` under the app's
data directory, written on every change, and the session set in memory.

| Builtin | Returns | Behaviour |
| --- | --- | --- |
| `storage::get_item(key: string)` | `any` | The stored text, or null. |
| `storage::set_item(key: string, value: string)` | `bool` | Store text; false when it could not be kept: over the browser's quota, or a data directory that cannot be written. |
| `storage::remove_item(key: string)` | | Remove a key. |
| `storage::keys()` | `string[]` | Every key. |
| `storage::clear()` | | Remove everything. |
| `storage::session_get_item(key: string)` | `any` | As `get_item`, in the session set. |
| `storage::session_set_item(key: string, value: string)` | `bool` | As `set_item`, in the session set. |
| `storage::session_remove_item(key: string)` | | As `remove_item`, in the session set. |
| `storage::session_keys()` | `string[]` | As `keys`, in the session set. |
| `storage::session_clear()` | | As `clear`, in the session set. |

In a page, another tab of the same site changing local storage arrives as
...
The tab that made the change is not told. A desktop app is one process with
one copy of the file, so there the event does not fire.
...
They live in their own `cookie` namespace.

| Builtin | Returns | Behaviour |
| --- | --- | --- |
| `cookie::get(name: string)` | `any` | The cookie's value, or null. |
| `cookie::set(name: string, value: string, options: {string: any})` | `bool` | Set a cookie; false when it was not kept, as a browser does not keep a `secure` cookie on an `http` page. |
| `cookie::remove(name: string, options: {string: any})` | | Remove a cookie. `path` and `domain` have to be the ones it was set with. |
| `cookie::keys()` | `string[]` | The name of every cookie the script can see. |

| Option | Value | Default |
| --- | --- | --- |
| `max_age` | seconds | until the browser closes, or the app exits |
| `path` | text | `/` |
| `domain` | text | the page's host; on the desktop, none |
...
raises, so a misspelled one is not quietly dropped. A cookie a server set
`HttpOnly` is invisible to the script: `get` does not find it and `remove`
cannot reach it.

In a page these are the cookies in `document.cookie`. On the desktop they are
a cookie jar that the app's own requests use: every
[`http()` and `fetch()`](#networking) request carries the cookies whose
domain, path and `secure` flag match its URL, and a `Set-Cookie` in the reply
goes into the jar, with the rules a browser applies. A request with
`"credentials": "omit"` neither sends nor keeps any. A cookie the script sets
goes to the requests of its `domain` and that domain's subdomains; without a
`domain` it stays in the jar for the script alone. Cookies with a `max_age`,
or an expiry a server gave them, are kept in `cookies.json` under the app's
data directory; the rest last as long as the run.
...
They live in their own `ws` namespace. A connection is named by a key the
script picks, and every call and event takes it first. An `http` or `https`
URL connects to the same place over `ws` or `wss`, and in a page a relative
URL connects to the site's own server. On the desktop each connection runs
on a thread of its own, `wss` is TLS against the Mozilla root certificates,
and events arrive on the tick after they happen.

| Builtin | Returns | Behaviour |
| --- | --- | --- |
| `ws::open(key: string, url: string)` | `bool` | Open a connection; false when one under `key` is still connecting or open. |
| `ws::send(key: string, text: string)` | `bool` | Send a text frame; false when the connection is not open. |
| `ws::close(key: string, code: int, reason: string)` | `bool` | Close the connection with a close code, 1000 for a normal close or 3000 to 4999; false when there is none. Another code raises. |
...
These functions come from the `lumen-js` runtime module, declared under
...
They live in their own `js` namespace and reach the page's own JavaScript, so
they do something only in a web build. On the desktop each one raises
`js::<function> runs only in a browser`; put a call behind
[`@cfg(web)`](#code-for-one-target) when the desktop build should do
something else.
...
A function that answers later takes a tag after its own arguments and answers
as `on_js_<function>(tag, value)`, or `on_js_<function>_error(tag, reason)`
when it fails.
...
To hand a part of the page to your own JavaScript, write a module with a web
half of your own that declares an [element](web-addons.md#elements): its
`mount` hook receives the element, and a library such as KaTeX or a code
editor draws into it.

## Browser

These functions come from the `lumen-browser` runtime module, declared under
`[dependencies]` in `lumen.toml`:
...
They live in their own `browser` namespace and reach what a browser offers a
page outside its own document, so they do something only in a web build. On
the desktop each one raises `browser::<function> runs only in a browser`.
Navigation is not here; [pages](../guides/pages.md) and links own it. A
function that answers later takes a tag and answers as
`on_browser_<function>(tag, value)`, or `on_browser_<function>_error(tag,
reason)`.
...
it is given one, and always on the desktop, it shows the content the markup
gives it:
...
The functions live in their own `svg` namespace and draw only in a web
build; on the desktop each one raises `svg::<function> runs only in a
browser`.
...
refuses to set one. A call naming an element that is not in the page yet is
kept and applied when the element arrives, and `set_attribute` then answers
0. The drawing fills the element's box.
