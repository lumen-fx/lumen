...
- **CSS grid week** - the grid is a CSS `display: grid` with
...
- **`derive()` stats** - `on_toggle` recomputes total checks, the longest run
  of perfect days, and completion percent. The header binds the raw counts
  directly and the formatted strings through `derive()`.
- **Settings dialog** - `<dialog open="settings_open">` with a goal
  `<slider>` and a clear-week button; `on_slider` recomputes goal progress.
- **`@media` width query** - below 760px the shell tightens its padding,
  shrinks the stat tiles, and narrows the habit-name column.
