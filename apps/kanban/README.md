...
- **Move between columns** — per-card `◀` / `▶` buttons carry an
  interpolated id (`ml|{row.id}`); the global `on_click` parses the prefix
  and shifts the card's `col`.
- **`derive()` counts** — each lane header and the board summary bind to
  derived labels recomputed from the per-column count signals.
- **Live search** — `bind-text` updates a `search` signal as you type; an
  `on_tick` watcher refilters when it changes.
