...
- `crates/primitives/src/lib.rs` - the existing primitives (baseline, checkbox,
...
- `crates/primitives/src/<thing>.rs` - Component plus its driving system.
...
- Two-world split: components mutating render-only data go in
  ...
  the extract step.
- Snapshot coverage: add the new component to `crates/mcp/src/plugin.rs`
  (the `snap_*` sweeps) and `crates/mcp/src/snapshot.rs` (an `EntityInspect`
...
- Tests: ship a unit test under the relevant crate, and a golden-image case in
  `crates/backends/render-headless/tests/` if the component renders.
