//! The request surface: `lumen::request_header` / `request_cookie` /
//! `request_body` read the request the document is being rendered for, and
...
fn returns(body: &str) -> Option<ScriptValue> {
    ...
    host.load(&src, "request.cdl").expect("load");
...
fn commands(body: &str) -> Vec<ScriptCommand> {
    ...
    let mut host = CandelaHost::new();
    host.load(&src, "request.cdl").expect("load");
    host.call("on_start", &[]).expect("call").commands
}
...
fn the_readers_reach_the_installed_request() {
    ...
    assert_eq!(
        returns("lumen::request_header(\"accept-language\")"),
        Some(ScriptValue::Str("en-GB".into()))
    );
    assert_eq!(
        returns("lumen::request_cookie(\"session\")"),
        Some(ScriptValue::Str("abc".into()))
    );
    assert_eq!(
        returns("lumen::request_body()"),
        Some(ScriptValue::Str("{\"name\":\"ada\"}".into()))
    );
    assert_eq!(
        returns("window::location_query()"),
        Some(ScriptValue::Str("tab=posts".into()))
    );
    assert_eq!(
        returns("window::location_hash()"),
...
fn the_answer_builtins_queue_their_commands() {
    let cmds = commands(
        "lumen::response_status(404); lumen::response_header(\"x-cache\", \"miss\"); lumen::redirect(\"/login\");",
    );
    assert!(
        cmds.iter()
...
fn a_status_outside_the_http_range_is_clamped() {
    ...
    assert_eq!(statuses("lumen::response_status(7);"), vec![100]);
    assert_eq!(statuses("lumen::response_status(9000);"), vec![599]);
}
