...
fn returns(body: &str) -> Option<ScriptValue> {
    ...
    host.load(&format!("fn probe() {{ return {body}; }}"))
        .expect("load");
...
fn commands(body: &str) -> Vec<ScriptCommand> {
    let mut host = RhaiHost::new();
    host.load(&format!("fn on_start() {{ {body} }}"))
        .expect("load");
...
fn the_readers_reach_the_installed_request() {
    ...
    assert_eq!(
        returns(r#"request_header("accept-language")"#),
        Some(ScriptValue::Str("en-GB".into()))
    );
    assert_eq!(
        returns(r#"request_cookie("session")"#),
...
fn the_answer_builtins_queue_their_commands() {
    let cmds = commands(
        r#"response_status(404); response_header("x-cache", "miss"); redirect("/login");"#,
    );
    assert!(
        cmds.iter()
