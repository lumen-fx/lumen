//! Phase-5 rhai introspection bindings: `n.rect()` / `n.is_visible()` /
//! `n.z_index()` / `n.component(name)` and the `frame_info()` global read
//! the published introspection snapshot and marshal to rhai maps / scalars.
//!
//! Publishes a DOM index + an introspection snapshot directly (no runtime),
//! then evaluates a rhai script that reads them and surfaces the values via
//! `print`, mirroring the host-neutral parity the other hosts assert.
...
#[test]
fn rhai_introspection_reads_snapshot() {
    let _btn = publish_fixture();
    let src = r#"
        fn read() {
            let b = get_by_id("btn");
            print("w=" + b.rect().width);
            print("vis=" + b.is_visible());
            print("z=" + b.z_index());
