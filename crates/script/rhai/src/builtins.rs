...
//! `signals.foo.set(v)` chained accessors) are intentionally **not**
//! listed here: they dispatch on receiver type, which the text-only LSP
//! cannot resolve without type inference. Only top-level free functions
//! (the ones an author calls bare) belong in the table.
//!
//! The metadata *types* are host-neutral and live in
//! [`lumen_script::builtins`]; only this Rhai table is host-specific.
...
// Shorthand param constructors keep the table below readable.
...
///
/// Keep this in sync with the `register_fn` calls in
/// [`crate::RhaiHost::new`]; the `builtins_parity` test enforces it.
