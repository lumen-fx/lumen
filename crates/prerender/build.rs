//! Compiles the test fixture to `.cdlb` bytecode.
//!
//! A run installs the candela host, which carries no compiler, so the suite
//! needs an image from somewhere. Producing it here with the same
...
//! links is never part of a shipped runtime.
...
const FIXTURES: &[&str] = &["settles", "fetches", "components"];
