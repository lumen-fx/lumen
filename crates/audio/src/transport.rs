...
//! (a platform player, say) can report that instead; the ones in this workspace
//! use the clock, because it behaves the same with or without an output device
//! and so stays meaningful in a headless test.

use std::sync::Arc;
use std::sync::atomic::{AtomicBool, Ordering};
use std::time::{Duration, Instant};

use crate::{AudioSnapshot, PlaybackState};
...
impl Transport {
    ...
    /// back with [`Self::volume`] to apply it to a device.
