...
/// named `foo-bar`). Every loader that probes for a library by name uses
/// this list: the `[[plugins]]` path resolution here, the runtime-module
/// probe in `lumen-modules`, and `lumenc package`, which stages module
/// libraries into the same names the probes look for.
pub fn library_spellings(name: &str) -> Vec<String> {
