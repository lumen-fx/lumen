...
//! the entity tree recovers the same paths. Three callers need that walk: the
//! browser runtime, which binds each entity to the element carrying its path;
//! the snapshot a prerender run reads, which records what the app wrote onto
//! each node; and the build's read of what a `<for>` row's components
//! rendered. One walk, so they cannot disagree about what a path names.
//!
...
pub struct Visit<'a, C> {
    ...
    pub path: &'a NodePath,
    /// Its position among its parent's children, which is what a caller
    /// pairs it with the element it was spawned from by.
    pub index: u32,
