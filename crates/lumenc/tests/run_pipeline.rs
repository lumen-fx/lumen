...
"#;
    ...
    fn virt_scroll_profile() {
        ...
        // Spawned UI entities only; resource entities are filtered out so the
        // printed count tracks the grid.
        let entities = app
            .world
            .query_filtered::<Entity, bevy_ecs::prelude::Without<bevy_ecs::resource::IsResource>>()
            .iter(&app.world)
            .count();
        let mut times = Vec::with_capacity(FRAMES);
        for _ in 0..FRAMES {
...
"#;
    ...
    fn snapshot(app: &mut App) -> SpawnSnapshot {
        // Resources live on their own entities, so the count is filtered down
        // to the spawned UI tree. Otherwise it also tracks how many resources
        // each path happens to insert.
        let count = app
            .world
            .query_filtered::<Entity, bevy_ecs::prelude::Without<bevy_ecs::resource::IsResource>>()
            .iter(&app.world)
