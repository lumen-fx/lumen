...
//! an image from somewhere. Producing it here, with the same
//! `CandelaHost::compile_bytecode` a `lumenc build` calls, means the artifact
//! a host loads is always the one this toolchain emits: a candela artifact
//! format bump breaks the test loudly instead of leaving a checked-in blob to
//! rot.
//!
