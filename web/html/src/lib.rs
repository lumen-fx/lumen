//! The contract shared by the two halves of Lumen's web target.
//!
//! One half is the emitter, which turns a compiled app into static HTML.
//! The other is the runtime that boots in the browser and takes that HTML
//! over. They only agree if they agree exactly, so the things both must
//! know live here and nowhere else: which HTML element an IR tag becomes,
//! how a node is named, which `data-lm-*` attributes carry state, how an
//! address hangs off the site's base path, and the shape of the manifest and
//! seed files the runtime loads.
//!
//! Nothing here reads or writes files, and nothing here knows what a page
//! is. It is a vocabulary, not a pipeline.

#![forbid(unsafe_code)]
#![warn(missing_docs)]

pub mod attrs;
pub mod contract;
pub mod escape;
pub mod paths;
pub mod style;
pub mod tags;
pub mod urls;

pub use attrs::{
    ImageBox, MarkupRules, PixelSize, class_list, class_value, control_attrs, html_attrs,
    image_attrs, is_disableable, markup_rules, radio_attrs,
};
pub use contract::{
    AddonRef, DATA_LM, DATA_LM_AUX, DATA_LM_BASE, DATA_LM_CHECKED, DATA_LM_CONTRACT,
    DATA_LM_DISABLED, DATA_LM_DRAG_OVER, DATA_LM_FOREIGN, DATA_LM_HIDDEN, DATA_LM_KEY,
    DATA_LM_LOCALE, DATA_LM_PAGE, DATA_LM_PART, DATA_LM_SELECTED, DATA_LM_WIDGET,
    DEFAULT_ARTIFACT_FILE, DEFAULT_CSS_FILE, DEFAULT_JS_FILE, DEFAULT_MANIFEST_FILE,
    DEFAULT_WASM_FILE, Dir, ForeignElement, LM_CONTRACT_VERSION, Manifest, NavigationMode,
    NodePath, NodeSeed, PageInfo, PathError, PathStep, SEED_SCRIPT_ID, ScriptFormat, ScriptRef,
    Seed, SeedValue, UnsupportedSeedValue, web_names,
};
pub use escape::{escape_attr, escape_text};
pub use paths::{Visit, walk_nodes};
pub use style::{Emission, WebDecl, rewrite_property, style_value};
pub use tags::{
    Control, HtmlTag, LM_CLASS_PREFIX, MAPPED_TAGS, VOID_ELEMENTS, drawn_by_control, html_tag_for,
    is_void, lm_class, part_from_class,
};
