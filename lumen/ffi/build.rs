...
fn main() {
    ...
    let config = match cbindgen::Config::from_file(crate_dir.join("cbindgen.toml")) {
        Ok(c) => c,
        Err(e) => {
            println!("cargo:warning=lumen-ffi: cbindgen.toml unreadable: {e}");
            return;
        }
    };
    ...
    match builder.generate() {
        ...
        Err(e) => {
            println!(
                "cargo:warning=lumen-ffi: cbindgen generation failed: {e}; keeping prior `lumen_simple.h`"
            );
        }
    }
    ...
    let out_dir = match env::var("OUT_DIR") {
        Ok(d) => PathBuf::from(d),
        Err(_) => {
            println!("cargo:warning=lumen-ffi: OUT_DIR unset; skipping lumen.pc generation");
            return;
        }
    };
    ...
    match fs::read_to_string(&template_path) {
        Ok(template) => {
            ...
            if let Err(e) = fs::write(&pc_path, &rendered) {
                println!(
                    "cargo:warning=lumen-ffi: failed to write {}: {e}",
                    pc_path.display()
                );
            } else {
                println!(
                    "cargo:warning=lumen-ffi: wrote {} (set LUMEN_PREFIX to override prefix={prefix})",
                    pc_path.display()
                );
            }
        }
        Err(e) => {
            println!(
                "cargo:warning=lumen-ffi: lumen.pc.in unreadable: {e}; skipping lumen.pc generation"
            );
        }
    }
}
