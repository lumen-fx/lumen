...
//! [`rodio`] output stream + `Sink`. Its shape mirrors Qt's
...
//!
//! rodio's `Sink` hands the decoded [`rodio::Source`] to cpal's audio
...
struct Output {
    // Field order matters for drop: the `Sink` must drop before the
    // `OutputStream` that backs its mixer.
    sink: Sink,
...
/// shaped. Held as a `NonSend` resource because rodio's `OutputStream`
...
/// `Sink::get_pos`) keeps headless tests meaningful and sidesteps the
...
pub struct AudioService {
    ...
    /// natural end or a `stop()` (both of which `sink.clear()` and thus drop
    /// the decoded source) can re-append a fresh `Decoder` instead of
    /// playing a `try_seek`-on-empty-sink silence.
    bytes: Option<Arc<[u8]>>,
    /// True when the sink's decoded source has been dropped (`stop()` or a
...
impl AudioService {
    ...
    pub fn new() -> Self {
        let output = match OutputStreamBuilder::open_default_stream() {
            Ok(stream) => {
    ...
    /// `OutputStream`, nor a background thread. An app that *does* use audio
    ...
    pub fn play_bytes(&mut self, bytes: Arc<[u8]>) -> Result<(), AudioError> {
        ...
        // Capture duration before the source is moved into the sink.
        ...
        if let Some(out) = &mut self.output {
            out.sink.clear(); // drop any previous track, leaves sink paused
            out.sink.set_volume(self.volume);
    ...
    /// Rebuild the sink's decoded source from the retained encoded bytes.
    ...
    pub fn resume(&mut self) {
        ...
        if self.cleared || at_end {
            ...
            // `try_seek(0)` on the now-empty sink plays silence while the
    ...
    pub fn stop(&mut self) {
        ...
        // `resume()` rebuilds it instead of playing an empty sink.
        self.cleared = true;
        if let Some(out) = &self.output {
            out.sink.clear();
        }
    ...
    pub fn seek(&mut self, secs: f64) {
        ...
        if let Some(out) = &self.output {
            let _ = out.sink.try_seek(target);
        }
    }
    ...
    pub fn refresh(&mut self) -> AudioSnapshot {
            ...
            && self.raw_position() >= self.duration
        {
            ...
            // resume-from-end rebuilds it instead of playing an empty sink.
            self.cleared = true;
            if let Some(out) = &self.output {
                out.sink.clear();
            }
