...
/// land here as the parser learns more rules; today the only one
/// is `BareInterpolation` for legacy `{name}` placeholders.
#[derive(Debug, Clone, Copy, PartialEq, Eq, serde::Serialize, serde::Deserialize)]
pub enum LintKind {
...
pub struct Attributes {
    ...
    /// directive generated by the `<tabs>` parser pass for the
    /// default-active tab. `(signal_name, default_value)`.
    ...
    pub required: bool,
    /// `pattern="<substring>"` - content must contain this literal substring to be valid.
