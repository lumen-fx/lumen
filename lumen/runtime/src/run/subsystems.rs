...
pub(crate) fn register_os_hotkey(app: &mut App) {
    ...
    if let Some(reg) = OsHotkeyRegistry::new() {
        app.world.insert_non_send_resource(reg);
...
pub(crate) fn register_os_tray(app: &mut App) {
    ...
    // crate from now on.
    app.world.insert_non_send_resource(OsTrayService::new());
...
///
/// The service is NonSend (rodio's OutputStream wraps a !Send cpal stream). A
...
pub(crate) fn register_audio(app: &mut App, audio_used: bool) {
    app.world.insert_non_send_resource(if audio_used {
