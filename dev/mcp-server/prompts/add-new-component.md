...
- `core/primitives/src/lib.rs` - the existing primitives (baseline, checkbox,
...
- `core/primitives/src/<thing>.rs` - Component plus its driving system.
- `core/primitives/src/lib.rs` - plugin registration and exports.
- `core/runtime/src/spawn.rs` - wire the new tag or attribute to the
  component.
- `public/lumenc/src/parse/html.rs` - parse the markup form if it is new.
- `dev/lsp/src/docs.rs` - add the tag or attribute to `TAGS` /
...
- Two-world split: components mutating render-only data go in
  ...
  the extract step.
- Snapshot coverage: add the new component to `core/mcp/src/plugin.rs`
  (the `snap_*` sweeps) and `core/mcp/src/snapshot.rs` (an `EntityInspect`
