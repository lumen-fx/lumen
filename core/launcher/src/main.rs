...
//! involved; see `docs/src/guides/packaging.md`.
//!
...
fn main() -> ExitCode {
    ...
    let outcome =
        lumenc::link::loader::run_via_dlopen(&bytes, &base_dir, headless.then_some(ticks))
...
/// `lumenc::link::loader::run_via_dlopen` drives across the dlopen seam, with the
