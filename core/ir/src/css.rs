...
//! `parse::css` front-end so the parser-free runtime and every other IR
...
//! `lumenc::parse::css` and produces the [`Stylesheet`] type defined here;
...
/// shape every shipped skin file uses (`core/runtime/src/skins/*.css`).
///
...
/// therefore stay in `lumenc`) can exercise it via the `parse::css`
/// re-export.
...
/// `pub` so the markup parser (`lumenc::parse::html`, a separate crate)
