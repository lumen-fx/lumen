...
//!
//! `apps/multi-host` pairs `model.cdl` (writes the `shared` signal) with
...
fn app_dir(name: &str) -> std::path::PathBuf {
    std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join(name)
        .canonicalize()
        .unwrap_or_else(|e| panic!("apps/{name} fixture must exist: {e}"))
}
...
/// `[script] engine` still collapses an app onto one host. `apps/lua-smoke`
