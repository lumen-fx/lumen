...
- The `msi-smoke` workflow is green. It installs the package on a Windows
  runner, checks the file layout and the user PATH entry, upgrades to a second
  version and confirms Windows records one product rather than two, then
  uninstalls and confirms nothing is left behind. It runs on every pull
  request that touches `tools/msi/` or either release workflow, and it can be
