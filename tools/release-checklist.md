...
upload step. Asset names are load-bearing, because `tools/install.sh` looks
them up verbatim, so the workflow generates them rather than anyone typing
them.
...
- The `msi-smoke` workflow is green. It installs the package on a Windows
  ...
  runs on every pull request that touches `tools/msi/`, `msi-smoke.yml`, or
  `release.yml`, and it can be started by hand from the Actions tab.
